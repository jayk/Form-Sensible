...
you collect the information in it's raw form and boil it down to something
...
is invalid, we simply redisplay the form. If you are particularly astute, you
may be wondering why we didn't loop through our error fields and generate
error messages. The answer is simple: we don't have to. Our renderer is smart
enough to recognize that validation has already been run and it inspects the
results and generates appropriate messaging on it's own.
...
is marked invalid. The last element, C<code> is the most interesting. It's
...
then the field itself. The subroutine is expected to return either a zero
    ...
    validation => {
        ...
        code => sub {
                        ...
                        if ($value ne $field->name) {
                            return $field->name . " is a bizarre field that can only contain its own name.  Field is invalid.";
                        } else {
...
Finally we need to cover error messaging.  For code based validation of either Fields or the entire form,
...
Render hints are a way to help the renderer decide how to render the field or form you are working with.  Render
hints can be placed into the form definition itself or can be inserted at various points along the rendering path.
Generally speaking, the hints that are closest to final rendering take precedence over those at a higher distance.
That is to say that if you have render_hints provided for the field in the C<render_field()> call, they will override
any hints set in the fields original definition.
